...
import { Proxy, proxyPool } from "./proxyPool.ts";
...
export class GoogleScholar implements IApiFetcher {
  ...
  public constructor(url: string, token: string, cache?: Cache<IApiResponse>) {
    ...
    this._userAgent =
      HttpUserAgents[Math.floor(Math.random() * HttpUserAgents.length)];
  }
  ...
  public async fetch(query: IApiQuery): Promise<IApiResponse> {
    if (!CONFIG.googleScholar.enabled) {
      return warnApiDisabledByConfig("GoogleScholar");
    }
    ...
    try {
      ...
      let rawHtml = await this._rateLimitedScrapeRequest(
        `${this.url}/scholar?as_sdt=0,5&q=${query.doi}&hl=en`,
        true,
      );
      let html: any = this._domParser.parseFromString(rawHtml, "text/html");

      paper = this._parseResponse(
        this._transformScrapedData(html.querySelector("#gs_res_ccl_mid > div")),
      );

      try {
        var citationUrl = html.querySelector(
          "#gs_res_ccl_mid > div > div > div.gs_fl > a:nth-child(3)",
        ).attributes.href;
        var citationCount = Number(
          html.querySelector(
            "#gs_res_ccl_mid > div > div > div.gs_fl > a:nth-child(3)",
          ).innerHTML.replace("Cited by ", ""),
        );
      } catch (e) {
  ...
  private _transformScrapedData(dom: any) {
    let title = dom.querySelector(".gs_ri > h3 > a")
      ? dom.querySelector(".gs_ri > h3 > a")
      : dom.querySelector(".gs_ri > h3 > span:not(.gs_ctu)");
    let info = dom.querySelector(".gs_a");
    let citedLink = dom.querySelector(".gs_fl > a:nth-child(3)");
    try {
      var splittedInfo = info.innerHTML.replace(/&nbsp;/g, " ").replace(
        /<\/?[^>]+(>|$)/g,
        "",
      ).split(" - ");
    ...
    return {
      ...
      "publisher": variousData && variousData.length > 1
        ? variousData[0].trim()
        : undefined,
      "year": variousData
        ? variousData[variousData.length - 1].trim()
        : undefined,
   ...
   */
  private async _rateLimitedScrapeRequest(
    url: string,
    ...
    let timeout = getRandomFromRange(
      CONFIG.googleScholar.requestInterval.min,
      CONFIG.googleScholar.requestInterval.max,
    );
    let timeDelta = lastScrappingRun
      ? difference(lastScrappingRun, new Date()).seconds!
      : timeout;
    ...
    if (timeDelta < timeout) {
      logger.warning(
        `GoogleScholar: globally ratelimiting requests. Waiting ${timeout} seconds...`,
      );
      await sleep(timeout - timeDelta);
    }
    let html = await fetch(
      url,
      this._proxy!.getFetchConfig(this._lastRefererUrl, this._currentCookie),
    );
    //console.log(html)
    ...
    if (html.status !== 200) {
      if (this._retries < 50) {
        await this._rotateProxy(
          `GS:Site blocked or not reachable. Http status code ${html.status}. Retry #${this._retries}.`,
        );
        return this._rateLimitedScrapeRequest(
          url,
          refererNeeded ? refererNeeded : undefined,
        );
      } else {
    ...
    if (!this._currentCookie) {
      this._currentCookie = html.headers.get("set-cookie")!;
    }
    if (refererNeeded) {
    ...
    if (parsed.querySelector("#gs_captcha_c")) {
      ...
      if (this._retries < 50) {
        await this._rotateProxy(
          `GS: Detected Captcha. Retry #${this._retries}.`,
        );
        return this._rateLimitedScrapeRequest(
          url,
          refererNeeded ? refererNeeded : undefined,
  ...
  private async _getCitations(
    ...
    if (
      numberOfCitations >
        (CONFIG.googleScholar.maxCitationCount >= 0 &&
            CONFIG.googleScholar.maxCitationCount <= 1000
          ? CONFIG.googleScholar.maxCitationCount
          : 1000)
    ) {
      logger.warning(
        `GS: Found ${numberOfCitations} citations.But only shows max 1000 citations in detailed view.We will ignore the rest.`,
      );
      numberOfCitations = 1000;
    }
    let iterations = numberOfCitations / 20;
    logger.info(
      `GS: Got ${numberOfCitations} citations.This will need ${iterations} fetches`,
    );
    ...
    while (iterations > 0) {
      ...
      iterations--;
      let rawHtml = await this._rateLimitedScrapeRequest(
        `${this.url}${url}&num=20&start=${currentPage}`,
      );
      currentPage += 20;
      let html: any = this._domParser.parseFromString(rawHtml, "text/html");
  ...
  private _parseResponse(response: any): IApiPaper {
    ...
    let parsedResponse: IApiPaper = {
      ...
      abstract: [],
      numberOfReferences: [],
      numberOfCitations: response.citationCount
        ? [Number(response.citationCount)]
        : [],

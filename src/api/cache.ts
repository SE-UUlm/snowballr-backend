...
export class Cache<V> {
  ...
  public constructor(
    ...
    if (type === CacheType.IM) {
      this.memoryCache = new RemoteCache.Cache<string, string>(ttl! * 1000);
    } else if (type === CacheType.F) {
      this.fileCache = new FileCache(
        `${
          new URL(".", import.meta.url).pathname
        }/../../fileCache/${folderName}`,
        ttl,
        checkInterval,
      );
  ...
  public async add(key: string, value: V) {
    if (this.memoryCache) {
      this.memoryCache.add(String(key), JSON.stringify(value));
    }
  ...
  public get(key: string): V | undefined {
    ...
    if (this.fileCache) {
      const result = this.fileCache.get(key);
      if (result) return JSON.parse(result);
    }
  }

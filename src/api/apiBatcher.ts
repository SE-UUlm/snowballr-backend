...
import { BatcherStatus, IApiBatch, IApiBatcher } from "./iApiBatcher.ts";
...
export class ApiBatcher implements IApiBatcher {
  ...
  public constructor() {
    ...
    for (const s in this._apiMapper) {
      this.cache[s] = new Cache<IApiResponse>(
        CONFIG.cache.type,
        CONFIG.cache.timeToLiveInSeconds,
        s.toString(),
      );
    }
  }
  ...
  public async startFetch(query: IApiQuery): Promise<IApiBatch> {
    const initializedFetchers: IApiFetcher[] = this._initializeEnabledApis(
      query.enabledApis!,
    );
  ...
  private async _mergerWorker(query: IApiQuery, response: IApiResponse[]) {
    ...
    const mergedPapers: Promise<IApiResponse[]> = new Promise<IApiResponse[]>(
      (resolve) => {
        const checkIfDone = () => {
          if (done) {
            resolve(data);
              ...
              checkIfDone,
              CONFIG.batcher.workerPollingRateInMilSecs,
            );}
        };
        checkIfDone();
      },
    ...
    const workerInit: Promise<void> = new Promise<void>((resolve) => {
      const checkIfDone = () => {
        if (initialized) {
          ...
          resolve(
            worker.postMessage(
              JSON.stringify({ query: query, response: response }),
            ),
          );
        } else {
   ...
   */
  private async _getDoiByFetching(
    query: IApiQuery,
    initializedFetchers: IApiFetcher[],
  ): Promise<IApiQuery> {
    try {
  ...
  private _initializeEnabledApis(apis: [SourceApi, string?][]): IApiFetcher[] {
    ...
    for (const a of apis) {
      ...
      initializedFetchers.push(
        new ApiObject(
          params[0] ? params[0] : "",
          params[1] ? params[1] : "",
          this.cache[a[0] as SourceApi],
        ),
      );
    }
  ...
  public stopFetch(batch: IApiBatch): boolean {
    ...
    if (included) {
      this.activeBatches = this.activeBatches.filter((item) =>
        !(item.id === batch.id)
      );
      return true;
    }
  ...
  public purge() {
    ...
    Object.keys(this.cache).forEach((key) => {
      if (this.cache[key].fileCache) this.cache[key].fileCache!.purge();
    });

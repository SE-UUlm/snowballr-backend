...
export class ApiMerger implements IApiMerger {
    ...
    public async compare(response: Promise<IApiResponse>[]): Promise<IApiResponse[]> {
        ...
        while (response.length > 1) {
            ...
            /*
            If the first papers can be merged with another paper, the merged paper will be put on the position of the second paper.
            If the first can't be merged with any other paper, it will be put on the finished stack.
            */
     ...
     * @returns list of merged papers with the position it should be put into the list
     */
    ...
    private _isEqual(firstResponse: IApiPaper, secondResponse: IApiPaper): boolean {
         ...
         * Weight is used to control the importance of the whole equality formula */
        const [sameAbstract, worked2] = this.compareStringArrays(comparison.abstractWeight, firstResponse.title, secondResponse.title)
        if(!worked2){
    ...
    private _mergeAuthors(firstAuthors: IApiAuthor[], secondAuthors: IApiAuthor[]): IApiAuthor[] {
        ...
        mergingAuthors.push.apply(firstAuthors.filter(item => item), secondAuthors);
...
// TODO: first name might be shortened while last name is most likely not shortened. handle this via a heuristic
    private _isEqualAuthor(firstAuthor: IApiAuthor, secondAuthor: IApiAuthor) {
        ...
        let s2 = secondAuthor.rawString.filter;
        if (s1 && s2) {

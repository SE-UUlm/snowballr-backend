...
export class ApiMerger implements IApiMerger {
  ...
  static normalizeString(pattern: string): string {
    if (typeof pattern === "string") {
      return pattern.toLowerCase().replace(/[äüö,\\\-:/\.]/g, " ").replace(
        / +/g,
        " ",
      ).trim();
    }
  ...
  public async compare(
    ...
    while (response.length > 1) {
      ...
      const finalPaper = await this.comparePaperWithPapers(
        response.shift()!,
        response,
      );
   ...
   */
  public async comparePaperWithPapers(
    response: Promise<IApiResponse>,
    others: Promise<IApiResponse>[],
  ): Promise<{ position: number; item: IApiResponse }> {
    ...
    for (let i: number = 0; i < others.length; i++) {
      ...
      const isEqual: boolean = isEqualPaper(
        (await response).paper,
        (await others[i]).paper,
        this.comparisonWeight,
      );
      if (isEqual) {
        ...
        /** Comparison for each citation with the citations of the other paper*/
        const response1Citations = this._getCiteOrRefList(
          (await response).citations,
        );
        const response2Citations = this._getCiteOrRefList(
          (await others[i]).citations,
        );

        /** Same for references*/
        const response1References = this._getCiteOrRefList(
          (await response).references,
        );
        const response2References = this._getCiteOrRefList(
          (await others[i]).references,
        );
        ...
        return {
          ...
          item: {
            paper: this.merge(
              (await response).paper,
              (await otherResponses).paper,
            ),
            citations: this._compareChildren(
              response1Citations,
              response2Citations,
            ),
            references: this._compareChildren(
              response1References,
              response2References,
            ),
          },
  ...
  public merge(firstResponse: IApiPaper, secondResponse: IApiPaper): IApiPaper {
    const uniqueProperties = this._selectUniqueKey(
      firstResponse,
      secondResponse,
    );
  ...
  private reviewPaper(finalChildren: IApiPaper[]) {
    ...
    for (let i: number = 0; i < finalChildren.length; i++) {
      ...
      for (let j: number = i + 1; j < finalChildren.length; j++) {
        ...
        if (isEqual) {
          logger.info(
            `CONCLUSIVE API paper merging: ${
              finalChildren[i].uniqueId!.map((item) =>
                item.type == idType.DOI ? item.value : undefined
              )
            } // ${finalChildren[i].title} <-> ${
              finalChildren[j].uniqueId!.map((item) =>
                item.type == idType.DOI ? item.value : undefined
              )
            } // ${finalChildren[j].title}`,
          );
   ...
   */
  private _compareChildren(
    response1Citations: IApiPaper[],
    response2Citations: IApiPaper[],
  ) {
    for (let i: number = 0; i < response1Citations.length; i++) {
      ...
      for (let j: number = 0; j < response2Citations.length; j++) {
        const isEqual: boolean = isEqualPaper(
          response1Citations[i],
          response2Citations[j],
          this.comparisonWeight,
        );
        if (isEqual) {
          logger.info(
            `DIFFERENT API paper merging: ${
              response1Citations[i].uniqueId!.map((item) =>
                item.type == idType.DOI ? item.value : undefined
              )
            } // ${response1Citations[i].title} <-> ${
              response2Citations[j].uniqueId!.map((item) =>
                item.type == idType.DOI ? item.value : undefined
              )
            } // ${response2Citations[j].title}`,
          );
          response2Citations[j] = this.merge(
            response1Citations[i],
            response2Citations[j],
          );
          response1Citations = response1Citations.slice(0, i).concat(
            response1Citations.slice(i + 1),
          );
          break;
        }
      }
   ...
   */
  private _mergeAuthors(
    firstAuthors: IApiAuthor[],
    secondAuthors: IApiAuthor[],
  ): IApiAuthor[] {
    ...
    for (let f1 = 0; f1 < firstAuthors.length; f1++) {
      for (let s1 = 0; s1 < secondAuthors.length; s1++) {
        ...
        if (val > this.comparisonWeight.overallWeight) {
          mergingAuthors.push(
            this._mergeAuthor(firstAuthors[f1], secondAuthors[s1]),
          );
          delete firstAuthors[f1];
          secondAuthors = secondAuthors.slice(0, s1).concat(
            secondAuthors.slice(s1 + 1),
          );
          break;
        }
      }
    }
    mergingAuthors = mergingAuthors.concat(
      firstAuthors.filter((item) => item),
      secondAuthors,
    );
   ...
   */
  private _mergeAuthor(
    firstAuthor: IApiAuthor,
    secondAuthor: IApiAuthor,
  ): IApiAuthor {
    ...
    for (const key in Object.assign({}, firstAuthor, secondAuthor)) {
      ...
      if (
        key === "id" || first[key] === undefined || second[key] === undefined
      ) {
        continue;
      }
      ...
      FIRSTLOOP: for (const i in first[key]) {
        ...
        for (const j in second[key]) {
          if (first[key][i] && second[key][j]) {
            if (
              ApiMerger.normalizeString(first[key][i]) ===
                ApiMerger.normalizeString(second[key][j])
            ) {
              mergedAuthor[key].push(
                this._deriveGenericProperty(first[key][i], second[key][j]),
              );
              second[key] = second[key].slice(0, j).concat(
                second[key].slice(j + 1),
              );
              continue FIRSTLOOP;
            }
          }
  ...
  private _deriveRawStringAuthor(mergedAuthor: IApiAuthor): IApiAuthor {
    if (
      mergedAuthor.firstName!.length === 1 &&
      mergedAuthor.lastName!.length === 1
    ) {
      mergedAuthor.rawString = [
        `${mergedAuthor.firstName![0]} ${mergedAuthor.lastName![0]}`,
      ];
    } else if (mergedAuthor.rawString!.length > 1) {
      //only leaves rawstrings that have a full name, not names like M. Muster
      const rawstring = mergedAuthor.rawString!.filter((item: string) =>
        !item.match(regexLetterFollowedByPoint)
      );
    ...
    if (!mergedAuthor.firstName[0] && mergedAuthor.rawString[0]) {
      if (mergedAuthor.rawString[0].includes(",")) {
        mergedAuthor.firstName.push(
          mergedAuthor.rawString[0].split(",")[1].trim(),
        );
      } else if (mergedAuthor.rawString[0].includes(" ")) {
        mergedAuthor.firstName.push(
          mergedAuthor.rawString[0].split(" ")[0].trim(),
        );
      }
    }
  ...
  private _selectUniqueKey(firstPaper: Object, secondPaper: Object): IApiPaper {
    ...
    try {
      for (const key in Object.assign({}, firstPaper, secondPaper)) {
        if (
          key == "id" || (!first[key] && !second[key]) || key.includes("Source")
        ) {
          continue;
        }
        if (key == "source") {
          resultingPaper[key] = concatWithoutDuplicates(
            first[key],
            second[key],
          );
          continue;
        ...
        if (first[key].length === 0) {
          ...
          key == "pdf" || typeof first[key][0] == "number" ||
          typeof second[key][0] == "number"
        ) {
          resultingPaper[key] = concatWithoutDuplicates(
            first[key],
            second[key],
          ...
          for (let i = 0; i < first[key].length; i++) {
            ...
            if (Array.isArray(resultingPaper[key + "Source"][position])) {
              resultingPaper[key + "Source"][position].push(
                first[key + "Source"][i][0],
              );
            } else {
              resultingPaper[key + "Source"][position] =
                first[key + "Source"][i];
            }
          }
          for (let i = 0; i < second[key].length; i++) {
          ...
          resultingPaper.author = this._mergeAuthors(
            first.author,
            second.author,
          );
          ...
          for (let i = 0; i < first[key].length; i++) {
            for (let j = 0; j < second[key].length; j++) {
              if (
                ApiMerger.normalizeString(first[key][i].value) ==
                  ApiMerger.normalizeString(second[key][j].value)
              ) {
          ...
          resultingPaper[key] = resultingPaper[key].concat(
            first[key].filter((item: string) => item),
          ).concat(second[key]);
        } else {
          resultingPaper[key] = [];
          resultingPaper[key + "Source"] = [];
          for (let i = 0; i < first[key].length; i++) {
            for (let j = 0; j < second[key].length; j++) {
              if (
                ...
                resultingPaper[key].push(
                  this._deriveGenericProperty(first[key][i], second[key][j]),
                );
                resultingPaper[key + "Source"].push(
                  [first[key + "Source"][i], second[key + "Source"][j]].flat(2),
                );
                delete first[key][i];
                ...
                second[key + "Source"] = second[key + "Source"].slice(0, j)
                  .concat(second[key + "Source"].slice(j + 1));
                break;
              }
            }
          }
          ...
          first[key + "Source"] = first[key + "Source"].filter((item: string) =>
            item
          );
          ...
          for (let i = 0; i < first[key].length; i++) {
            ...
            resultingPaper[key + "Source"].push(
              first[key + "Source"][i].flat(),
            );
          }
          for (let i = 0; i < second[key].length; i++) {
            resultingPaper[key + "Source"].push(
              second[key + "Source"][i].flat(),
            );
          }

          resultingPaper[key] = resultingPaper[key].concat(first[key]).concat(
            second[key],
          );
        }
      }
    } catch (er) {
      resultingPaper = firstPaper;
      console.log(er);

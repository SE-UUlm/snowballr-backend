 ...
 */
export const isEqualPaper = (
  firstResponse: IApiPaper,
  secondResponse: IApiPaper,
  ...
  const [sameTitle, worked1] = compareStringArrays(
    comparison.titleWeight,
    firstResponse.title,
   ...
   * Weight is used to control the importance of the whole equality formula */
  const [sameAbstract, worked2] = compareStringArrays(
    comparison.abstractWeight,
    firstResponse.abstract,
    secondResponse.abstract,
  );
  if (!worked2) {
  ...
  if (
    firstResponse.year && secondResponse.year &&
    firstResponse.year.length > 0 && secondResponse.year.length > 0
  ) {
    sameYear = compareYears(firstResponse.year, secondResponse.year)
      ? comparison.yearWeight
      : -comparison.yearWeight;
  } else {
   ...
   * Weight is used to control the importance of the whole equality formula */
  if (
    firstResponse.author && secondResponse.author &&
    firstResponse.author.length > 0 && secondResponse.author.length > 0
  ) { // 0.7 ->
    sameAuthor = isEqualAuthors(firstResponse.author, secondResponse.author) *
      comparison.authorWeight;
  ...
  if (
    ((sameTitle + sameAbstract + sameAuthor + sameYear) /
      (comparison.titleWeight + comparison.abstractWeight +
        comparison.authorWeight + comparison.yearWeight)) >
      comparison.overallWeight
  ) {
    return true;
  }
  return false;
...
const isEqualAuthors = (
  ...
  return (equalAuthors /
    (firstAuthors.length >= secondAuthors.length
      ? firstAuthors.length
      : secondAuthors.length));
};

export const isEqualAuthor = (
  firstAuthor: IApiAuthor,
  secondAuthor: IApiAuthor,
) => {
  if (
    Object.keys(firstAuthor).length === 0 ||
    Object.keys(secondAuthor).length === 0
  ) {
    logger.error("one author value was not set");
    return 0;
  }
  const fFirstName = firstAuthor.firstName!.map((item: string) =>
    ApiMerger.normalizeString(item)
  );
  const sFirstName = secondAuthor.firstName!.map((item: string) =>
    ApiMerger.normalizeString(item)
  );
  const fLastName = firstAuthor.lastName!.map((item: string) =>
    ApiMerger.normalizeString(item)
  );
  const sLastName = secondAuthor.lastName!.map((item: string) =>
    ApiMerger.normalizeString(item)
  );

  if (
    fFirstName.some((item: string) => sFirstName.includes(item)) &&
    fLastName.some((item: string) => sLastName.includes(item))
  ) {
    return 1;
  }
  ...
  if (
    s1.length === 0 && firstAuthor.firstName!.length > 0 &&
    firstAuthor.lastName!.length > 0
  ) {
    s1 = [`${firstAuthor.firstName![0]} ${firstAuthor.lastName![0]}`];
  }
  if (
    s2.length === 0 && secondAuthor.firstName!.length > 0 &&
    secondAuthor.lastName!.length > 0
  ) {
...
export const isEqualRawAuthorString = (
  firstRawString: string,
  secondRawString: string,
): number => {
  let equalParts: number = 0;
  const firstNormalizedItems: string[] = ApiMerger.normalizeString(
    firstRawString,
  ).split(" ");
  const secondNormalizedItems: string[] = ApiMerger.normalizeString(
    secondRawString,
  ).split(" ");

  //Special case a name is given like M. Muster or M Muster
  if (
    firstRawString.match(regexLetterFollowedByPoint) ||
    secondRawString.match(regexLetterFollowedByPoint)
  ) {
    ...
    if (
      ...
      const lev = Levenshtein(
        firstNormalizedItems[firstNormalizedItems.length - 1],
        secondNormalizedItems[secondNormalizedItems.length - 1],
      );
      return ((secondNormalizedItems[secondNormalizedItems.length - 1].length -
        lev) / secondNormalizedItems[secondNormalizedItems.length - 1].length);
    }
  } else {
  ...
  return equalParts /
    (firstNormalizedItems.length >= secondNormalizedItems.length
      ? firstNormalizedItems.length
      : secondNormalizedItems.length);
};
...
const compareStringArrays = (
  ...
  if (first && second && first[0] && second[0]) {
    ...
    const lev = Math.max.apply(
      null,
      first.map((item) => {
        return Levenshtein(
          ApiMerger.normalizeString(item),
          ApiMerger.normalizeString(title),
        );
      }),
    );

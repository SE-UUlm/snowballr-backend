...
export class CrossRefApi implements IApiFetcher {
  ...
  public async fetch(query: IApiQuery): Promise<IApiResponse> {
    ...
    try {
      ...
      this._rateInterval = Number(response.headers.get("x-rate-limit-interval"))
        ? Number(
          response.headers.get("x-rate-limit-interval")!.replace("s", ""),
        )
        : this._rateInterval;
      this._rateLimit = Number(response.headers.get("x-rate-limit-limit"))
        ? Number(response.headers.get("x-rate-limit-limit")!.replace("s", ""))
        : this._rateInterval;
      const json = await response.json();
      paper = this._parseResponse(json);
      references = json.message.reference
        ? this.getChildObjects(json.message.reference)
        : new Promise((resolve) => {
          resolve([]);
        });
      citations = json.message.relation && json.message.relation.cites
        ? this.getChildObjects(json.message.relation.cites)
        : new Promise((resolve) => {
          resolve([]);
        });
      apiReturn = {
  ...
  private _parseQuery(query: IApiQuery): string {
    ...
    if (query.title && query.rawName) {
      return `${this.url}?query.bibliographic=${
        query.title.replace(/\s/g, "+")
      }&query.author=${query.rawName.replace(/\s/g, "+")}&rows=1`;
    }
    if (query.title) {
      return `${this.url}?query.bibliographic=${
        query.title.replace(/\s/g, "+")
    ...
    throw new Error(
      "Cannot fetch for this query. We either need a doi or a title.",
    );
  }
  ...
  private async _fetchBibFromApi(item: string): Promise<IApiPaper> {
    try {
      const response = await fetch(
        `${this.url}?query.bibliographic=${item.replace(/\s/g, "+")}&rows=1`,
        {
          headers: this._headers,
        },
      );
  ...
  private async _limitRequests() {
    ...
    if (this._iterations === this._rateLimit) {
      ...
      this._iterations = 0;
      logger.warning(
        `Limiting Crossref Api due to following Restricitons: rateLimit=${this._rateLimit}; rateInterval=${this._rateInterval}`,
      );
    }
  }
  ...
  private _parseResponse(response: any): IApiPaper {
    ...
    const parsedResponse: IApiPaper = {
      ...
      abstract: [],
      numberOfReferences: response.message["references-count"]
        ? [response.message["references-count"]]
        : [],
      numberOfCitations: response.message["is-referenced-by-count"]
      ...
      scope: [],
      scopeName: [],
      pdf: response.message.link
        ? response.message.link.map((item: any) => item.URL)
        : [],
  ...
  public async getDoi(query: IApiQuery): Promise<string | undefined> {
    try {
      const url = `${this.url}?query.bibliographic=${
        query.title!.replace(/\s/g, "+")
      }&rows=1`;

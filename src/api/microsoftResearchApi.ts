...
export class MicrosoftResearchApi implements IApiFetcher {
  ...
  private static _convertInvertedAbstract(iA: any): string[] {
    ...
    try {
      ...
      logger.error(
        "Couldn't convert InvertedAbstract from Microsoft API to a single string: " +
          error,
      );
      //logger.critical(iA);
      return [];
  ...
  public async fetch(query: IApiQuery): Promise<IApiResponse> {
    if (!CONFIG.microsoftAcademic.enabled) {
      return warnApiDisabledByConfig("MicrosoftAcademic");
    }
    ...
    try {
      ...
      citations = json.entities[0] && json.entities[0].Id
        ? this._getCitations(json.entities[0].Id)
        : new Promise((resolve) => {
          resolve([]);
        });
      references = json.entities[0] && json.entities[0].RId
        ? this._getReferences(json.entities[0].RId)
        : new Promise((resolve) => {
          resolve([]);
        });
  ...
  private _parseQuery(query: IApiQuery): string {
    if (!((query.title && query.title.trim().length > 0) || query.doi)) {
      throw new Error(
        "Neither a doi nor a title is given. We cannot search for anything.",
      );
    }
    let urlQuery: string = `Or(DOI='${
      query.doi ? query.doi.toUpperCase() : ""
    }', Ti='${query.title ? query.title!.toLowerCase() : ""}')`;

    if (query.rawName && query.rawName.trim().length > 0 && query.title) {
      urlQuery = `Or(DOI='${
        query.doi ? query.doi.toUpperCase() : ""
      }', And(Composite(AA.AuN='${query.rawName!.toLowerCase()}'), Ti='${
  ...
  private _parseResponse(response: any): IApiPaper {
    ...
    const parsedResponse: IApiPaper = {
      ...
      abstract: response.IA
        ? MicrosoftResearchApi._convertInvertedAbstract(response.IA)
        : [],
  ...
  public async getDoi(query: IApiQuery): Promise<string | undefined> {
    try {
      if (!(query.title && query.title.trim().length > 0)) {
        throw new Error(
          "If no DOI is given, we need atleast a title for the search.",
        );
      }
      let urlQuery = `Composite(AA.AuN='${query.rawName!.toLowerCase()}')`;
      if (query.rawName) {
        urlQuery = `And(Composite(AA.AuN='${
          query.rawName!.toLowerCase()
        }'), Ti='${query.title!.toLowerCase()}')`;
      }
      const response = await fetch(this.url, {
        method: "POST",
        headers: this._headers,

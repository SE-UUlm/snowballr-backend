...
export class Proxy implements IProxy {
  ...
  public async block(): Promise<void> {
    throw new Error(
      "GS: roxy isnt working, is captchaed, or blocked. We cannot fetch with this config",
    );
  }
...
export class TorProxy extends Proxy implements IProxy {
  ...
  public async block(): Promise<void> {
    logger.warning(
      "GS: Tor Proxy got blocked. Reloading service to get new ip. Make sure the executing user is allowed to restart the tor docker container. See documentation for more info!",
    );
    this._userAgent =
      HttpUserAgents[Math.floor(Math.random() * HttpUserAgents.length)];
    ...
    if (status.success === false) {
      process.close();
      throw new Error(
        "GS: Error handling tor service. Run deno as admin and check for tor being installed.",
      );
    }
    let debugOutStr = "---";
    try {
      ...
      logger.info(
        `GS: Successfully restarted Tor Docker Container. New IP is ${debugOutStr}`,
      );
    }
    process.close();
  }
...
class ProxyManager {
  ...
  private constructor(proxyPorts?: number[], proxyAddresses?: number[]) {
    try {
      ...
      throw new Error(
        "GS: Cannot read config.yml from project source. Or config.yml isn't formatted correctly. Error: " +
          e.message,
      );
    }
    if (this._mode === "pool") {
      ...
      for (const p in CONFIG.googleScholar.proxy.urls) {
        const proxy = new GoogleScholarProxy(
          `${CONFIG.googleScholar.proxy.urls[p]}`,
        );
  ...
  public static Instance(proxyPorts?: number[], proxyAddresses?: number[]) {
    ...
    return this._instance ||
      (this._instance = new this(
        proxyPorts ? proxyPorts : undefined,
        proxyAddresses ? proxyAddresses : undefined,
      ));
  }
  ...
  public async acquire(): Promise<Proxy> {
    ...
    if (!this._settings.proxy.enabled) {
      logger.info(
        `GS: Proxy is disabled via config.yaml. Trying to run without a proxy`,
      );
      throw new Error(
        "GS: Tried to acquire a proxy. But proxy is disabled via config.",
      );
    }

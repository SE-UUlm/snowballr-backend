import {
  insertUser,
  returnUserByEmail,
} from "../controller/databaseFetcher/user.ts";
...
export const setup = async (dropDatabase: boolean) => {
  ...
  await db.sync({ drop: dropDatabase }).catch((err) => {
    ...
    console.log(err);
    console.log(
      "Entering workaround for: https://github.com/eveningkid/denodb/issues/258",
    );
  });
  ...
  if (!admin) {
    admin = await insertUser(
      String(Deno.env.get("ADMIN_EMAIL")),
      String(Deno.env.get("ADMIN_PASSWORD")),
      true,
      "admin",
      "admin",
      "active",
    ...
    await SearchApi.create({
      name: "crossRef",
      id: IDOfApi.crossRef,
      credentials: "luca999@web.de",
    });
    await SearchApi.create({
      name: "openCitations",
      id: IDOfApi.openCitations,
    });
    await SearchApi.create({
      name: "semanticScholar",
      id: IDOfApi.semanticScholar,
    });
    await SearchApi.create({
      name: "IEEE",
      id: IDOfApi.IEEE,
      credentials: "4yk5d9an52ejynjsmzqxe62r",
    });
    await SearchApi.create({
      name: "googleScholar",
      id: IDOfApi.googleScholar,
    });
    await SearchApi.create({
      name: "microsoftAcademic",
      id: IDOfApi.microsoftAcademic,
      credentials: "9a02225751354cd29397eba3f5382101",
    });

...
} from "../../controller/databaseFetcher/paper.ts";
...
} from "../../controller/databaseFetcher/review.ts";
import { PaperScopeForStage } from "../../model/db/paperScopeForStage.ts";
import { AuthorMessage } from "../../model/messages/author.message.ts";

export const convertPapersToPaperMessage = async (
  papers: Paper[],
  stageId?: number,
  userId?: number,
) => {
  const paperMessages: PaperMessage[] = [];
  for (const item of papers) {
    const paperMessage: PaperMessage = await convertPaperToPaperMessage(
      item,
      stageId,
      userId,
    );
...
export const convertRowsToPaperMessage = (
  answer: any,
  userID: number,
  paperCacheUrls: string[],
) => {
  const paperMessage: PaperMessage[] = [];
  let lastId = -1;
  for (const element of answer) {
    if (lastId == Number(element[0])) {
      const paper = paperMessage[paperMessage.length - 1];
...
export const convertPaperToPaperMessage = async (
  paper: Paper,
  stageId?: number,
  userId?: number,
) => {
  ...
  if (stageId) {
    ...
    if (pp) {
      ...
      if (pp.finalDecision) {
        paperMessage.finalDecision = String(pp.finalDecision);
      }
    }
  }
  if (paperCache.has(String(paper.id))) {
    ...
    userId && paperMessage.ppid &&
    await checkUserReviewOfProjectPaper(paperMessage.ppid, userId)
  ) {
    paperMessage.status = Status.evaluatedByMyself;
  } else if (
    paperMessage.ppid &&
    (await getAllReviewsFromProjectPaper(paperMessage.ppid)).length > 0
  ) {
    paperMessage.status = Status.partiallyEvaluated;
  } else {
    paperMessage.status = Status.ready;
  }
  const pdf = await Pdf.where({ paperId: Number(paper.id) }).get();
  ...
  authors.forEach((author) =>
    paperMessage.authors.push(convertAuthorToAuthorMessage(author))
  );
  const unfinishedAuthors = paperMessage.authors.some((author) =>
    author.status === Status.unfinished
  );
...
export const convertIApiPaperToDBPaper = async (
  paper: IApiPaper,
): Promise<Paper> => {
  ...
  for (const i in paper) {
    if (
      ["title", "abstract", "publisher", "type", "scope", "scopeName", "year"]
        .includes(i)
    ) {
      newPaper[i] = p[i][0];
    } else if (i == "uniqueId") {
      newPaper.doi = getDOI(paper)[0];
      const uniqueId = paper.uniqueId.filter((item: IApiUniqueId) =>
        item.type !== "DOI"
      );
...
export const convertDBPaperToIApiPaper = async (
  paper: Paper,
): Promise<IApiPaper> => {
  const pdfs: string[] = [];
  const pdf = await Pdf.where({ paperId: Number(paper.id) }).get();
  ...
  const authors: IApiAuthor[] = [];
  const dbAuthors = await getAllAuthorsFromPaper(Number(paper.id));
...
const addAuthorToDatabase = async (paper: IApiPaper, newPaper: Paper) => {
  for (const item of paper.author) {
    let author: Author;
    if (item.orcid[0]) {
      const oldAuthor = await Author.where({ orcid: item.orcid[0] }).get();
...
export const assignOnlyIfUnassignedPaper = async (
  target: Paper,
  source: IApiPaper,
) => {
  try {
...
const updateUniqueIDsOfPaper = (target: Paper, source: IApiPaper) => {
  ...
  let uniqueId = source.uniqueId.filter((item: IApiUniqueId) =>
    item.type !== "DOI"
  );
  for (let i = 0; i < uniqueId.length; i++) {
    if (
      uniqueId[i].type && uniqueId[i].value &&
      checkUniqueVal(String(uniqueId[i].type), String(uniqueId[i].value))
    ) {
      delete uniqueId[i];
    }
  }
...
const updateAuthorsOfPaper = async (target: Paper, source: IApiPaper) => {
  const authors = await getAllAuthorsFromPaper(Number(target.id));
  for (const element of authors) {
    const iAuthor: IApiAuthor = {
    ...
    source.author = source.author.filter((value) =>
      Object.keys(value).length !== 0
    );
    for (let i = 0; i < source.author.length; i++) {
      //TODO hardcoded
      if (isEqualAuthor(iAuthor, source.author[i]) > 0.9) {
        source.author = source.author.slice(0, i).concat(
          source.author.slice(i + 1),
        );
        break;
      }
    }
    ...
    for (const item of source.author) {
      await updateAuthorOfPaper(
        await Author.create({}),
        item,
        Number(target.id),
      );
...
const updatePdfOfPaper = async (target: Paper, source: IApiPaper) => {
  const pdf = Pdf.where({ paperId: Number(target.id) }).get();
  if (Array.isArray(pdf)) {
    const urls: string[] = [];
    pdf.forEach((item) => {
      urls.push(String(item.url));
    });
    for (const item of source.pdf) {
 ...
 * @param newPaperId
 */
const addUniqueToPaper = async (
  uniqueId: IApiUniqueId[],
  newPaperId: number,
) => {
  for (const item of uniqueId) {
    const paperId = await PaperID.create({});
 ...
 * @param paperId
 */
const updateAuthorOfPaper = async (
  author: Author,
  item: IApiAuthor,
  paperId: number,
) => {
...
export const checkIApiPaper = (paper: { [index: string]: any }): boolean => {
  ...
  for (const i in paper) {
    ...
    if (
      ...
      if (
        (paper[i] && paper[i].length > 1) ||
        (i === "raw" && paper[i].length > 0)
      ) {
        check = false;
      } else {
        delete paper[i];

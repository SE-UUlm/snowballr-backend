...
private class StructureRules {
    companion object {
        ...
        private const val NORMALIZATION = "Normalization"
    ...
    private fun Architectures.LayeredArchitecture.snowballRLayers() = this
        ...
        // Paper normalization, shared by the Fetcher and Service layers
        .layer(NORMALIZATION)
        .definedBy("$BASE_PACKAGE.normalization..")
        // REST API
        .layer(REST)
    ...
    fun `When the layer architecture is violated, then this test should fail (only layer deps)`(classes: JavaClasses) {
        layeredArchitecture()
            ...
            // Checks
            .whereLayer(MAIN)
            .mayOnlyAccessLayers(SERVICE, ACCESS, REPO, DB, SCHEDULER, FETCHER, MATCHING, REST)
            .whereLayer(VALIDATION)
            .mayNotAccessAnyLayer()
            .whereLayer(GRPC)
            .mayOnlyAccessLayers(SERVICE, VALIDATION)
            .whereLayer(SERVICE)
            ...
            .mayOnlyAccessLayers(REPO)
            .whereLayer(FETCHER)
            .mayOnlyAccessLayers(REPO, MATCHING, NORMALIZATION)
            .whereLayer(MATCHING)
            .mayNotAccessAnyLayer()
            .whereLayer(NORMALIZATION)
            .mayNotAccessAnyLayer()
            .whereLayer(REST)
            .mayOnlyAccessLayers(SERVICE, GRPC)
            .check(classes)
    }
...
private class NamingConventions {
    ...
    fun `When a class is in the repository package, then it should have the 'TableRepo' suffix`(classes: JavaClasses) {
        classes()
            ...
            .orShould(haveSimpleName("SqlFunctionsKt")) // exception

...
export const addToReadingList = async (ctx: Context, userID: number) => {
    const validate = await validateUserEntry(ctx, [userID], UserStatus.needsSameUser, -1, { needed: true, params: ["id"] }, userID)
    if (validate) {
        try {
            const reading = await ReadingList.create({ userId: userID, paperId: validate.id })
...
export const removeFromReadingList = async (ctx: Context, userID: number, readingID: number) => {
    const validate = await validateUserEntry(ctx, [userID, readingID], UserStatus.needsSameUser, -1, { needed: false, params: [] }, userID)
    if (validate) {
...
export const getReadingList = async (ctx: Context, userID: number) => {
    const validate = await validateUserEntry(ctx, [userID], UserStatus.needsSameUser, -1, { needed: false, params: [] }, userID)
    if (validate) {

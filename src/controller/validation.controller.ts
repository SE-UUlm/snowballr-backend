...
import { create, decode, verify } from "https://deno.land/x/djwt@v2.9/mod.ts";
import { User } from "../model/db/user.ts";
import {
  createNumericTerminationDate,
  createTokenExpiration,
} from "../helper/dateHelper.ts";
  ...
  if (await emptyContent(ctx)) {
    ...
    if (await validateContent(ctx)) {
      ...
      makeErrorMessage(
        ctx,
        415,
        "The requestBody is not in a correct JSON format",
      );
    }
  } else {
 ...
 * @param next
 */
export const validateJWTIfExists = async (
  ctx: Context,
  next: () => Promise<unknown>,
) => {
 ...
 * @returns
 */
export const checkPOofProject = async (
  projectID: number,
  payloadJson?: PayloadJson,
) => {
  if (payloadJson) {
    const userProject = await UserIsPartOfProject.where({
      userId: payloadJson.id,
      projectId: projectID,
    }).get();
 ...
 * @returns
 */
export const checkMemberOfProject = async (
  projectID: number,
  payloadJson?: PayloadJson,
) => {
  if (payloadJson) {
 ...
 * @param ctx
 */
export const getPayloadFromJWTHeader = async (
  ctx: Context,
): Promise<PayloadJson | undefined> => {
...
export const getPayloadFromJWTCookie = async (
  ctx: Context,
): Promise<PayloadJson | undefined> => {
  const token = await ctx.cookies.get("refreshToken");
  if (token) {
 ...
 * @param token
 */
const verifyJWTAndContinue = async (
  ctx: Context,
  next: () => Promise<unknown>,
  token: string,
) => {
 ...
 */
const allowedAddressesUnauthorized = async (
  ctx: Context,
  next: () => Promise<unknown>,
) => {
  if (
    ctx.request.url.pathname === "/login/" ||
    ctx.request.url.pathname === "/reset-password/" ||
    ctx.request.url.pathname === "/refresh/" ||
    (ctx.request.url.pathname.match(/\/users\/[0-9]+[\/]?/g) &&
      ctx.request.method.toString() === "PATCH")
  ) {
...
export const validateUserEntry = async (
  ...
  needed: UserStatus,
  projectID: number,
  requestParameter: { needed: boolean; params: string[] },
  userID?: number,
) => {
    ...
    for (const param of requestParameter.params) {
      if (params[param] === undefined) {
        ...
        makeErrorMessage(
          ctx,
          422,
          `Request doesn't include parameter ${param}`,
        );
        return;
      }
    }
...
const checkAuthorization = async (
  ...
  if (needed === UserStatus.needsSameMemberOfProject) {
    if (
      !(await checkMemberOfProject(projectID, payloadJson) &&
        (await getUserID(payloadJson) === userID))
    ) {
      makeErrorMessage(ctx, 401, "not authorized");
      return;
    }

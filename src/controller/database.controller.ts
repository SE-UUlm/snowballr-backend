...
export const getChildren = (
  ...
  return client.queryArray(
    `select p.* from ${table} as i JOIN paper as p ON i.${column2} = p.id WHERE i.${column1} = ${id}`,
  );
};

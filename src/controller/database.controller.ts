...
SELECT i.id, i.final_decision, i.addition_date, p.id as paper_id, p.doi, p.title, p.abstract, p.year, p.publisher, p.type, p.scope, p.scope_name, pdf.url, a.id, a.raw_string, a.first_name, a.last_name, a.orcid , r.overall_evaluation, r.userId FROM inscopefor as i JOIN paper as p ON i.paper_id = p.id LEFT OUTER JOIN pdf ON p.id = pdf.paper_id LEFT OUTER JOIN wrote as w ON w.paper_id = p.id LEFT OUTER JOIN author as a ON a.id = w.author_id LEFT OUTER JOIN review as r ON r.paper_id = i.id WHERE i.stage_id = ${id} ORDER BY i.id`)
}
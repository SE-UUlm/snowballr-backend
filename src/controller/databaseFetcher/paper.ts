...
export const getAllPapersFromStage = async (
  id: number,
): Promise<
  { paper: Paper; scope: PaperScopeForStage; authors: Author[] }[]
> => {
  ...
  if (Array.isArray(paperScopes)) {
    const paperPromises: {
      paper: Paper;
      scope: PaperScopeForStage;
      authors: Author[];
    }[] = [];
    for (const item of paperScopes) {
      paperPromises.push({
        paper: (await Paper.find(Number(item.paperId))) as Paper,
        scope: item,
        authors: await getAllAuthorsFromPaper(Number(item.paperId)),
      });
    }
...
export const getAllPapersFromProject = async (
  id: number,
): Promise<
  {
    papers: { paper: Paper; scope: PaperScopeForStage; authors: Author[] }[];
    stage: Stage;
  }[]
> => {
  ...
  for (const stage of stages) {
    const papers = {
      papers: await getAllPapersFromStage(Number(stage.id)),
      stage: stage,
    };
...
export const getProjectPaperScope = async (
  stageId: number,
  paperId: number,
) => {
  const paperScope = await PaperScopeForStage.where({
...
export const checkPaperInProjectStage = async (
  paper: Paper,
  stageID: number,
) => {
  const pp = await PaperScopeForStage.where({

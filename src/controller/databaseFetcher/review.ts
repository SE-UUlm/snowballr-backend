...
export const getAllReviewsFromProjectPaper = async (ppId: number): Promise<ReviewMessage[]> => {
	...
	try {
		const scope = await ReviewToPaperScope.where({ paperscopeforstageId: ppId }).get();
...
const getCriteriaEvalsOfCriteria = async (reviewID: number, message: ReviewMessage) => {
	const ces = await CriteriaEvaluation.where(CriteriaEvaluation.field("review_id"), reviewID).get()

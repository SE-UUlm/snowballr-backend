...
export const getAllReviewsFromProjectPaper = async (
  ppId: number,
): Promise<ReviewMessage[]> => {
  // let reviews = await Review.where(Review.field("paperscopeforstage_id"), ppId).get()
  console.log(
    "HEREREREREREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE",
  );
  const reviews: Review[] = [];
  try {
    const scope = await ReviewToPaperScope.where({ paperscopeforstageId: ppId })
      .get();
...
export const checkUserReviewOfProjectPaper = async (
  ppId: number,
  userId: number,
): Promise<boolean> => {
  ...
  try {
    ...
    if (Array.isArray(scope)) {
      for (const s of scope) {
        reviews.push(
          ((await Review.where({ id: Number(s.reviewId), userId: userId })
            .get()) as Review[])[0],
        );
      }
    }
...
export const getReviews = async (reviews: Review | Review[]) => {
  if (Array.isArray(reviews)) {
    const arr = new Array<ReviewMessage>();
    for (const review of reviews) {
      ...
      if (review.overallEvaluation) {
        message.overallEvaluation = String(review.overallEvaluation);
      }
      if (review.finishDate) {
        message.finishDate = new Date(String(review.finishDate));
      }
...
const getCriteriaEvalsOfCriteria = async (
  reviewID: number,
  message: ReviewMessage,
) => {
  const ces = await CriteriaEvaluation.where(

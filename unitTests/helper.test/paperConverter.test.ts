...
Deno.test({
  ...
  async fn() {
    try {
      ...
      let author: IApiAuthor[] = [{
        rawString: ["This Author"],
        orcid: [],
        firstName: [],
        lastName: [],
      }, {
        rawString: ["Other Author"],
        orcid: [],
        firstName: [],
        lastName: [],
      }];
...
Deno.test({
  ...
  async fn() {
    try {
      await setup(true);
      const oldAuthor = await Author.create({
        orcid: "1234",
        rawString: "this author",
      });

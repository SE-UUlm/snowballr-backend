...
import {
  ...
  patchAuthor,
  postAuthor,
} from "../../src/controller/author.controller.ts";
...
Deno.test({
  ...
  async fn(): Promise<void> {
    ...
    const ctx = await createMockContext(
      app,
      `{"firstName":"This", "lastName":"Works", "rawString": "This Works", "orcid":"12ab"}`,
...
Deno.test({
  ...
  async fn(): Promise<void> {
    ...
    const source: IApiAuthor = {
      rawString: ["test tester", "testing tester"],
    } as IApiAuthor;
...
Deno.test({
  ...
  async fn(): Promise<void> {
    ...
    await authorCache.add(
      String(author.id),
      { rawString: ["test tester", "testing tester"] } as IApiAuthor,
    );
    const token = await createJWT(user);
    const ctx = await createMockContext(
      app,

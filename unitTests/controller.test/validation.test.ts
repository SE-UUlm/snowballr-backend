...
Deno.test({
    ...
    async fn(): Promise<void> {
        const app = await createMockApp();
        const ctx = await createMockContext(app, `{"email": "test@test", "password":"ash"}`, [["Content-Type", "text"]]);
        await validateContentType(ctx, emptyAsyncFunctionTest)
...
Deno.test({
    ...
    async fn(): Promise<void> {
        const app = await createMockApp();
        const ctx = await createMockContext(app, `{"email": "test@test", "password":"ash"}`, [["Content-Type", "text"]], "/login/");
...
Deno.test({
    ...
    async fn(): Promise<void> {
        ...
        const ctx = await createMockContext(app, undefined, [["Content-Type", "text"]], "/", token);
        await validateJWTIfExists(ctx, emptyAsyncFunctionTest)
...
Deno.test({
    ...
    async fn(): Promise<void> {
        ...
        const app = await createMockApp();
        const token = await createJWT(user)
        const ctx = await createMockContext(app, undefined, [["Content-Type", "text"]], "/", token + "a");
        await validateJWTIfExists(ctx, emptyAsyncFunctionTest)

...
Deno.test({
  ...
  async fn(): Promise<void> {
    ...
    let counter = 0;
    stub(globalThis, "fetch", () => {
      const body = new Blob([responseList[counter++]], {
    ...
    assertEquals(res, {
      ...
      "citations": Mock.parsedCitations,
      "references": [],
    });
  },
});
...
Deno.test({
  ...
  async fn(): Promise<void> {
    ...
    stub(GS, "_rotateProxy", () => {
      proxyExchanged = true;
      throw new Error("Cancelled by unittest");
    });

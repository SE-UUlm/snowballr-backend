...
Deno.test({
	...
	async fn(): Promise<void> {
		const cache: Cache<IApiResponse> = new Cache<IApiResponse>(CacheType.IM, 60, "test");
...
Deno.test({
	...
	async fn(): Promise<void> {
		const cache: Cache<IApiResponse> = new Cache<IApiResponse>(CacheType.F, 10080, "test");
...
Deno.test({
	...
	async fn(): Promise<void> {
		...
		const cache2: Cache<IApiResponse> = new Cache<IApiResponse>(CacheType.F, 10080, "test");
...
Deno.test({
	...
	async fn(): Promise<void> {
		const cache: Cache<IApiResponse> = new Cache<IApiResponse>(CacheType.IM, 2, "test");
		await cache.add("1", Mock.apiResponse);
		await sleep(4);
		const cacheValue = cache.get("1");
		//console.log(cacheValue)
...
Deno.test({
	...
	async fn(): Promise<void> {
		const cache: Cache<IApiResponse> = new Cache<IApiResponse>(CacheType.IM, 2, "test", 1000);
		await cache.add("1", Mock.apiResponse);
		await sleep(4);
		const cacheValue = cache.get("1");
		//console.log(cacheValue)

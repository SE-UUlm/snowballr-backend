...
import {
  ...
  getActiveBatches,
  makeFetching,
} from "../../src/controller/fetch.controller.ts";
...
Deno.test({
  ...
  async fn(): Promise<void> {
    await setup(true);
    const batch = await makeFetching(
      0.8,
      [
        [SourceApi.CR, "luca999@web.de"],
        [SourceApi.IE, "4yk5d9an52ejynjsmzqxe62r"],
        [SourceApi.MA, "9a02225751354cd29397eba3f5382101"],
        [SourceApi.OC],
        [SourceApi.S2],
      ],
      undefined,
    ...
    assertEquals(
      getDOI((await batch.response)[0].paper)[0].toUpperCase(),
